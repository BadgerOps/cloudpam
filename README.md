...
| Document | Description |
|----------|-------------|
| API Reference | Interactive API reference at `/openapi`; raw OpenAPI 3.1 spec at `/openapi.yaml` |
...
CloudPAM provides a REST API served at `/api/v1/`. The authoritative OpenAPI spec is generated by the running server from registered routes. View it interactively at `/openapi`, or fetch the raw machine-readable spec from `/openapi.yaml`.
...
- `GET /openapi` / `GET /openapi.yaml` - Interactive API reference and raw OpenAPI spec

...
### Fixed
- Server Docker image now builds the Vite frontend before `go build`, so release images embed the generated `web/dist/assets/*` bundles instead of shipping only `index.html`
- SPA asset handling now returns `404` for missing asset-like paths instead of falling back to `index.html`, preventing misleading `text/html` MIME errors for JavaScript module requests
- Added regression coverage for missing `/assets/*.js` requests to keep the SPA fallback limited to client-side routes
